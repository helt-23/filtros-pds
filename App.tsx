...
const App: React.FC = () => {
  ...
  // State Management
  // ==========================================
  const [activeTab, setActiveTab] = useState<"experiment" | "report">(
    "experiment"
  );

  // Source & Filter Configuration
  const [selectedSource, setSelectedSource] = useState<ImageSource>(
    ImageSource.SYNTHETIC_COMBINED
  );
  ...
  const getActiveKernel = useCallback((): Kernel3x3 | null => {
    ...
    if (activeFilter === FilterType.SOBEL_MAG)
      return KERNELS[FilterType.SOBEL_X];
    if (activeFilter === FilterType.PREWITT_MAG)
      return KERNELS[FilterType.PREWITT_X];
  ...
  const processImage = useCallback(() => {
    ...
    try {
      ...
      if (activeFilter === FilterType.NONE) {
        ...
        const sx = applyConvolution(
          processingData,
          KERNELS[FilterType.SOBEL_X].matrix,
          0
        );
        const sy = applyConvolution(
          processingData,
        ...
        const px = applyConvolution(
          processingData,
          KERNELS[FilterType.PREWITT_X].matrix,
          0
        );
        const py = applyConvolution(
          processingData,
  ...
  const handleGenerateReport = async () => {
    const originalImg =
      originalCanvasRef.current?.toDataURL("image/png") || null;
    const processedImg =
      processedCanvasRef.current?.toDataURL("image/png") || null;
    const spectrumImg =
      spectrumCanvasRef.current?.toDataURL("image/png") || null;

    setActiveTab("report");

    if (selectedSource !== ImageSource.UPLOAD) {
      const customKernel =
        activeFilter === FilterType.LOG ? getActiveKernel() : undefined;
      const staticReport = getStaticReport(
        selectedSource,
        activeFilter,
        {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1 font-medium">
                Tema 2.2: Detecção de Bordas & Padrões
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in duration-500">
            ...
            <div className="lg:col-span-3 space-y-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select
                      ...
                      value={
                        selectedSource === ImageSource.UPLOAD
                          ? ""
                          : selectedSource
                      }
                      onChange={(e) => {
                        if (e.target.value)
                          setSelectedSource(e.target.value as ImageSource);
                      }}
                    >
                      <option value={ImageSource.SYNTHETIC_COMBINED}>
                        Formas Combinadas
                      </option>
                      <option value={ImageSource.SYNTHETIC_CIRCLE}>
                        Círculo (Curvas)
                      </option>
                      <option value={ImageSource.SYNTHETIC_RECT}>
                        Retângulo (Quinas)
                      </option>
                    </select>
                  </div>
                  ...
                  <div className="pt-2">
                    <label className="block text-sm font-medium text-gray-700 mb-1 flex items-center gap-1">
                      <Waves size={14} className="text-gray-500" /> Adicionar
                      Ruído (Teste)
                    </label>
                    <div className="grid grid-cols-1 gap-2">
                      <button
                        onClick={() => setActiveNoise(NoiseType.NONE)}
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => setActiveNoise(NoiseType.GAUSSIAN)}
                          className={`flex-1 text-xs px-2 py-1.5 rounded border ${
                            activeNoise === NoiseType.GAUSSIAN
                        ...
                        <button
                          onClick={() => setActiveNoise(NoiseType.SALT_PEPPER)}
                          className={`flex-1 text-xs px-2 py-1.5 rounded border ${
                            activeNoise === NoiseType.SALT_PEPPER
                              ? "bg-orange-600 text-white"
                              : "bg-white"
                          }`}
                        >
                    ...
                    <div className="flex items-start gap-1 mt-2">
                      ...
                      <p className="text-[10px] text-gray-500">
                        Ruído Gaussiano: aleatório suave. Salt & Pepper: pixels
                        pretos/brancos aleatórios.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="relative py-2">
                    ...
                    <div className="relative flex justify-center">
                      <span className="bg-white px-2 text-xs text-gray-400 uppercase">
                        ou enviar
                      </span>
                    </div>
                  </div>
                ...
                {activeFilter !== FilterType.NONE && (
                  <div className="mb-4 p-3 bg-gray-50 rounded border border-gray-200">
                    ...
                    <div className="flex items-start gap-1 mt-1">
                      ...
                      />
                      <p className="text-[10px] text-gray-500">
                        Valores baixos preservam bordas fracas. Valores altos
                        removem ruído.
                      </p>
                    </div>
                  </div>
                  ...
                  {[
                    {
                      ...
                      val: FilterType.SOBEL_MAG,
                      desc: "Gradiente + suavização leve",
                    },
                    {
                      label: "Prewitt (Magnitude)",
                      val: FilterType.PREWITT_MAG,
                      desc: "Gradiente simples",
                    },
                    {
                      label: "Laplacian",
                      val: FilterType.LAPLACIAN,
                      desc: "Derivada segunda",
                    },
                    {
                      label: "LoG (Laplacian of Gaussian)",
                      val: FilterType.LOG,
                      desc: "Suavização + bordas",
                    },
                  ].map((opt) => (
                    <div key={opt.val}>
                      <button
                        onClick={() => setActiveFilter(opt.val)}
                        className={`w-full text-left px-3 py-2.5 rounded-md text-sm flex items-center justify-between ${
                  ...
                  activeFilter !== FilterType.NONE && (
                    <div className="mt-4 p-3 bg-gray-50 rounded border border-gray-200">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <button
                          onClick={() =>
                            setEnablePreGaussian(!enablePreGaussian)
                          }
                        ...
                        <div className="mt-2 animate-in slide-in-from-top-2">
                          <div className="flex justify-between items-center mb-1">
                            <span className="text-xs text-gray-600">
                              Intensidade (σ): {preSigma.toFixed(1)}
                            </span>
                          </div>
                          <input
                            type="range"
                            ...
                            value={preSigma}
                            onChange={(e) =>
                              setPreSigma(parseFloat(e.target.value))
                            }
                          ...
                          <div className="flex items-start gap-1 mt-1">
                            ...
                            />
                            <p className="text-[10px] text-gray-500">
                              Suaviza a imagem antes da detecção de bordas.
                              Reduz ruído mas pode borrar bordas finas.
                            </p>
                          </div>
                        </div>
                      )}
              ...
              <button
                ...
                className="w-full bg-slate-900 text-white py-4 rounded-xl shadow-lg hover:bg-slate-800 flex items-center justify-center gap-2"
              >
                {isSynthetic ? <Zap size={18} /> : <Cpu size={18} />}{" "}
                {isSynthetic
              ...
              <div className="p-4 bg-blue-50 rounded-lg border border-blue-100">
                ...
                <p className="text-xs text-blue-700">
                  1. Comece sem ruído para ver o comportamento ideal
                  <br />
                  2. Adicione ruído e ajuste o limiar
                  <br />
                  3. Use pré-suavização para filtros simples com muito ruído
                  <br />
            ...
            <div className="lg:col-span-6 space-y-6">
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="px-4 py-3 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
                  ...
                  <span className="text-[10px] px-2 py-1 bg-gray-200 text-gray-600 rounded-full font-mono">
                    512 x 512 px
                  </span>
                </div>
                <div className="p-6 flex flex-col gap-6 bg-gray-50/30">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <canvas
                      ...
                      className="w-full aspect-square bg-white rounded-lg border-2 border-gray-200 shadow-md"
                    />
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <canvas
                      ...
                      className="w-full aspect-square bg-black rounded-lg border-2 border-blue-200 shadow-md"
                    />
                    {activeFilter === FilterType.NONE && (
                  ...
                  <div className="mt-2 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
                    ...
                    {activeFilter === FilterType.LOG ? (
                      <div className="animate-in slide-in-from-top-2">
                        ...
                        <div className="mt-3 p-2 bg-blue-50 rounded border border-blue-100">
                          <p className="text-xs text-blue-800">
                            ...
                            Combina suavização gaussiana com detecção de bordas
                            Laplacianas. Sigma controla a intensidade da
                            suavização:
                            <br />•{" "}
                            <span className="font-semibold">
                              Sigma baixo (0.5-1.5):
                            </span>{" "}
                            Bordas finas e precisas, sensível a ruído
                            <br />•{" "}
                            <span className="font-semibold">
                              Sigma alto (2.0-5.0):
                            </span>{" "}
                            Bordas grossas, forte supressão de ruído
                          </p>
                        </div>
                      </div>
                    ) : activeFilter !== FilterType.NONE &&
                      ...
                      <div className="text-center p-4 text-gray-600">
                        ...
                        />
                        <p className="text-sm">
                          Pré-suavização ativa com σ = {preSigma.toFixed(1)}
                        </p>
                        <p className="text-xs text-gray-500">
                          A suavização é aplicada antes do filtro de borda
                        </p>
                      </div>
                    ) : activeFilter !== FilterType.NONE ? (
                      <div className="text-center p-4 text-gray-400">
                        ...
                        <p className="text-sm">
                          Ative a pré-suavização ou selecione LoG para ajustar
                          suavização
                        </p>
                      </div>
                    ) : (
                      <div className="text-center p-4 text-gray-400">
                        ...
                        <p className="text-sm">
                          Selecione um filtro para ver os parâmetros
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="space-y-8 flex-1">
                  ...
                  <div className="p-3 bg-gray-50 rounded border border-gray-200 text-xs text-gray-600">
                    ...
                    <p>
                      ...
                      Resposta concentrada no centro
                    </p>
                    <p>
                      •{" "}
                      <span className="font-semibold">
                        Filtros passa-alta (bordas):
                      </span>{" "}
                      Resposta nas bordas
                    </p>
                    <p>
                      •{" "}
                      <span className="font-semibold">LoG (passa-banda):</span>{" "}
                      Resposta em anel
                    </p>
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
                ...
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">
                      Projeto Final PDS 2025.4 - Unifesspa
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <button
              ...
              <div className="p-8 min-h-[500px]">
                {isGeneratingReport ? (
                  ...
                  <div
                    className="prose prose-slate max-w-none prose-headings:text-slate-900 prose-p:text-slate-700 prose-a:text-blue-600"
