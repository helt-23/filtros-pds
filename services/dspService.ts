...
export const generateLoGKernel = (sigma: number): Kernel3x3 => {
  ...
  return {
    ...
    matrix,
    description: `Laplacian of Gaussian. Sigma=${sigma.toFixed(1)}. Kernel ${size}x${size}. Soma ≈ ${scaledSum.toFixed(3)}`
  };
};
...
export const drawSyntheticImage = (ctx: CanvasRenderingContext2D, type: string, width: number, height: number) => {
...
export const applyNoise = (imageData: ImageData, type: NoiseType): ImageData => {
...
export const applyConvolution = (
  ...
  for (let r of kernelMatrix) {
    for (let v of r) {
      ...
      maxAbsWeight = Math.max(maxAbsWeight, Math.abs(v));
    }
  }
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      for (let ky = 0; ky < kRows; ky++) {
        for (let kx = 0; kx < kCols; kx++) {
          ...
          if (px >= 0 && px < width && py >= 0 && py < height) {
            ...
            // Usa apenas o canal vermelho (assumindo imagem em tons de cinza)
            rSum += srcData[offset] * weight;
            gSum += srcData[offset + 1] * weight;
            bSum += srcData[offset + 2] * weight;
          }
        }
      }
      ...
      let result: number;
      if (isHighPass) {
        // Magnitude da resposta (todos os canais iguais para tons de cinza)
        const magnitude = Math.sqrt(rSum * rSum + gSum * gSum + bSum * bSum);
        result = Math.min(255, magnitude / normalizationFactor);
      } else {
        // Para Gaussian, usa o valor diretamente
        result = Math.min(255, Math.max(0, rSum));
      }
...
export const generateGaussianKernel = (sigma: number): Kernel3x3 => {
    ...
    const matrix: number[][] = Array(actualSize).fill(0).map(() => Array(actualSize).fill(0));
    let sum = 0;
    ...
    return { 
      ...
      matrix, 
      description: `Filtro passa-baixa para suavização. Sigma=${sigma.toFixed(1)} controla intensidade.`
    };
};
...
export const computeKernelSpectrum = (kernel: number[][], size: number = 128): number[][] => {
    const spectrum: number[][] = Array(size).fill(0).map(() => Array(size).fill(0));
    const cx = size / 2;
    const cy = size / 2;
